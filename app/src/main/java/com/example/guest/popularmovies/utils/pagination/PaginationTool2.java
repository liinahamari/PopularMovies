...
/**
 * Created by l1maginaire on 3/18/18.
 */
...
public class PaginationTool2<T> {
    ...
    private PaginationTool2() { //todo: savedinstancestate and position of scrolling
    }
    ...
    public Observable<T> getPagingObservable() {
        ...
        return getScrollObservable(recyclerView, emptyListCount)
                ...
                .switchMap(new Function<Integer, ObservableSource<? extends T>>() {
                    @Override
                    public ObservableSource<? extends T> apply(Integer offset) throws Exception {
                        return PaginationTool2.this.getPagingObservable(pagingListener,
...
//                            int offset = emptyListCountPlusToOffset ? recyclerView.getAdapter().getItemCount() : recyclerView.getAdapter().getItemCount() - emptyListCount;
    ...
    private int getLastVisibleItemPosition(RecyclerView recyclerView) {
        ...
        if (recyclerViewLMClass == LinearLayoutManager.class || LinearLayoutManager.class.isAssignableFrom(recyclerViewLMClass)) {
            ...
            throw new PagingException("Wrong LayoutManager class: " + recyclerViewLMClass.toString());
        }
    }
    ...
    public static class Builder<T> {
        ...
        public Builder<T> setEmptyListCountPlusToOffset(boolean emptyListCountPlusToOffset) {
            ...
            public PaginationTool2<T> build() {
            PaginationTool2<T> paginationTool = new PaginationTool2<>();
